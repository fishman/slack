...
void WebView::loadFinished(bool success)
{
  ...
  const char * javascript =
      ...
      "window.createNotification = function(icon, title, body) { foo.someMethod(title,body) };"
      "var foo = channel.objects.foo;"
      "});"
      "Notification = function(title, args) {"
      "   args = args || {};"
      "   window.createNotification(args.iconUrl || '', title, args.body || '');"
      "   return this;"
      "};"
